...
pred t5[] {
some a1 : Address | some a2 : Address | some s1 : XState | some R : XState |
some cf : CacheFlush | some r1 : Read | some r2 : Read | some reg : REG  |
  s1 != R and r1 != r2 and
  (cf.address = a1) and (cf.xstate_access.xstate = s1) and (r1.address = a1) and (r1.xstate_access.xstate = s1)
  and (r2.address = a1) and (r2.xstate_access.xstate = s1) and (reg.address = a2) and (reg.xstate_access.xstate = R) and
  (cf->reg in eo) and (cf->r2 in po) and (reg->r1 in eo) and (r1->r2 in eo) and
  (reg->r1 in addr) and (cf ->r1 in erfx)  and (cf->r1 in ecox) and (cf->r1 in efrx) and (r1->r2 in erfx) and
  (cf->r1 in rf_init) and (cf->r2 in rf_init)  
}
