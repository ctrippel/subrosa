...
some cf : CacheFlush | some r1 : Read | some r2 : Read | some r3 : Read | some w : Event  |
  a0 != a1 and s0 != s1 and w != cf and w != r1 and w != r2 and w != r3 and r1 != r2 and r1 != r3 and r2 != r3 and
  (cf.address = a1) and (cf.xstate_access.xstate = s1) and (w.address = a0) and (w.xstate_access.xstate = s0) and (r1.address = a0) and (r1.xstate_access.xstate = s0)
  and (r2.address = a1) and (r2.xstate_access.xstate = s1) and (r3.address = a1) and (r3.xstate_access.xstate = s1) and
  (cf->w in po) and (cf->w in eo) and (w->r1 in eo) and (r1->r2 in eo) and (r2->r3 in eo) and (w->r3 in po) and
  (r1->r2 in addr) and (cf ->r2 in erfx)  and (cf->r2 in ecox) and (cf->r2 in efrx) and (r2->r3 in erfx) and (r1->w in efrx) and
